...
namespace Vexe.FastSave
{
	...
	public class FSMarker : BaseBehaviour, ISerializationCallbackReceiver
	{
		...
		[SerializeField, Hide]
		private List<string> _SerializedList = new List<string>();
...
#if UNITY_EDITOR
		private const string kMarkSelectionKey = "&#m";    // Alt-Shift-m
		private const string kUnmarkSelectionKey = "&#u";  // Alt-Shift-u

		/// <summary>
		/// Marks all the components on this gameObject to be saved
		/// </summary>
		[Show("Editor")]
		private void MarkAllComponentsForSave()
		{
		...
		/// </summary>
		[Show("Editor")]
		private void UnmarkThisAndChildren()
		{
			UnmarkChildren();
		...
		private static void OnMarkerGUI(int id, Rect rect)
		{
